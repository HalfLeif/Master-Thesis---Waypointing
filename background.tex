...
We now describe how attention works in more technical detail.
The goal of the attention model is to produce a vector $z \in \matbb{R}^D$ that represents the entire input image by weighing different parts of the activation map from the encoder CNN.
The activation map contains $L$ units $a_i \in \matbb{R}^D$.
Each unit $a_i$ is a feature vector for a specific location in the input image. We feed each $a_i$ to an MLP which produces a single number $e_i$. If the decoder uses an RNN, its hidden state is also used as input to the MLP besides the feature vector $a_i$.
