...
A common version of weight decay is \textbf{L2 regularization}. It adds a penalty to the

\[
\Omega(\mathbf{w}) = \frac{1}{2} \mathbf{w}^T \mathbf{w}
\]

By computing the gradient for $\Omega$ we get the new update rule for the weights:
...
We see that $\mathbf{w}$ decay with a factor $(1 - \eta \alpha)$ in each iteration. Thus the weights decay exponentially over time. Thus we see that the weight decay makes sure that the weights are kept small no matter for how long the network is trained, that is no weight parameter can grow indefinitely. Empirically, it has been observed that networks can be trained longer without overfitting when the weights are bounded \cite[Chapter~3]{NielsenBook}.
...
A more commonly used loss function is \textbf{cross-entropy} which has the property that the learning rate is proportional to the current error.
