...
% In SGD we do not calculate $H$ exactly as a sum over $D$, but as a sum over a random subset $B$
%but in each iteration draw a random sample $B$ from $D$ of a fixed size. The sample $B$ is called a \textbf{mini-batch}.
% instead sum over a random sample of $D$ called a \textbf{mini-batch}.
% It is common to randomly partition the dataset $D$ into mini-batches so that no datapoint is repeated.

% It is common to divide the dataset into mini-batches before training so that all mini-batches are disjoint.

% In order to get avoid getting stuck in local minima, it is common to apply \textbf{stochastic gradient descent} instead. This is achieved by summing over a small subset of $D$ instead of the entire data set.
